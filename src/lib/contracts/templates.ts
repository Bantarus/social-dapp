...
export const HALL_CONTRACT_TEMPLATE = (placeholders: HallContractParams) => 
`@version 1
...
export function generateHallContract(placeholders: Partial<HallContractParams>): string {
  // Default values
  const defaults: HallContractParams = {
  ...
  return HALL_CONTRACT_TEMPLATE(finalPlaceholders)
} 